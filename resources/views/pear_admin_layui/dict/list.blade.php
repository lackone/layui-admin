        ...
        layui.use(['treeTable', 'dropdown', 'laydate', 'form', 'util', 'jquery'], function () {
            ...
            treeTable.render({
                ...
                cols: [[
                    ...
                    },
                    {
                        ...
                        title: '类型',
                        templet: function (d) {
                            if (d.type == '{{ \App\Models\Dict::TYPE_RAW }}') {
                                return '原始内容';
                            } else if (d.type == '{{ \App\Models\Dict::TYPE_JSON }}') {
                                return 'JSON';
                            } else {
                                return '';
                            }
                    ...
                    },
                    {
                        ...
                        title: '状态',
                        templet: function (d) {
                            if (d.status == '{{ \App\Models\Dict::STATUS_ENABLE }}') {
                            ...
                            switch (obj.id) {
                                ...
                                case '{{ getDomIdKey('', 'update') }}':
                                    ...
                                    layer.open({
                                        ...
                                        type: 2,
                                        area: ['80%', '80%'],
                                        content: '{{ route('admin.dict.save') }}/' + data[0].id
                                    });
                                    break;
            ...
            treeTable.on('toolbar({{ getDomIdKey('', 'table') }})', function (obj) {
                ...
                switch (obj.event) {
                    case 'custom_export':
                        var field = form.val('{{ getDomIdKey('', 'table_filter') }}');
                        location.href = '{{ route('admin.dict.list') }}?export=1&' + $.param(field);
                        break;
                }
            });
            ...
            treeTable.on('tool({{ getDomIdKey('', 'table') }})', function (obj) { // 双击 toolDouble
                ...
                if (obj.event === 'edit') {
                    layer.open({
                        ...
                        type: 2,
                        area: ['80%', '80%'],
                        content: '{{ route('admin.dict.save') }}/' + data.id
                    });
                    ...
                    layer.open({
                        ...
                        type: 2,
                        area: ['80%', '80%'],
                        content: '{{ route('admin.dict.save') }}' + '?pid=' + data.id
                    });
