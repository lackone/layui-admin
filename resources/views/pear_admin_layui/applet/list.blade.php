        ...
        <div class="layui-card-body">
            <form class="layui-form layui-row layui-col-space16" lay-filter="{{ getDomIdKey('', 'table_filter') }}">
                <div class="layui-form-item">
                    @include('component.text', ['label' => '小程序名称', 'name' => 'name', 'is_search' => 1])

                    @include('component.text', ['label' => '小程序appid', 'name' => 'appid', 'is_search' => 1])
...
@section('myjs')
    ...
    <script type="text/html" id="{{ getDomIdKey('', 'status_templet') }}">
        <!-- 这里的 checked 的状态值判断仅作为演示 -->
        <input type="checkbox" name="status" value="<%= d.id %>" title="启用|禁用" lay-skin="switch"
               lay-filter="{{ getDomIdKey('', 'status') }}" <%= d.status == 1 ? "checked" : "" %>>
    </script>
    <script>
        layui.use(['table', 'dropdown', 'laydate', 'form', 'util', 'jquery'], function () {
            ...
            table.render({
                ...
                cols: [[
                    ...
                    },
                    {
                        ...
                        title: '小程序名称',
                        templet: function (d) {
                            var str = '名称 : ' + d.name + '<br>';
                            str += '原始id : ' + d.oid + '<br>';
                            str += '绑定pid : ' + d.pid + '<br>';
                            str += 'appid : ' + d.appid + '<br>';
                            str += 'appsecret : ' + d.appsecret + '<br>';
                            return str;
                        }
                    },
                    {
                        field: 'status',
                        title: '状态',
                        templet: '#{{ getDomIdKey('', 'status_templet') }}'
                    },
                    {
                        field: 'created',
                        title: '创建时间/更新时间',
                            ...
                            switch (obj.id) {
                                case '{{ getDomIdKey('', 'add') }}':
                                    layer.open({
                                        ...
                                        type: 2,
                                        area: ['80%', '80%'],
                                        content: '{{ route('admin.applet.save') }}'
                                    });
                                    break;
                                case '{{ getDomIdKey('', 'update') }}':
                                    ...
                                    layer.open({
                                        ...
                                        type: 2,
                                        area: ['80%', '80%'],
                                        content: '{{ route('admin.applet.save') }}/' + data[0].id
                                    });
                                    break;
                                case '{{ getDomIdKey('', 'delete') }}':
                                    ...
                                    layer.confirm('真的删除么?', function (index) {
                                        $.post("{{ route('admin.applet.delete') }}", {
                                            id: ids
                                        }, function (res) {
                                            if (res.code == 200) {
            ...
            // 状态 - 开关操作
            form.on('switch({{ getDomIdKey('', 'status') }})', function (obj) {
                var id = this.value;
                var name = this.name;
                var val = obj.elem.checked ? "{{ \App\Models\Applet::STATUS_ENABLE }}" : "{{ \App\Models\Applet::STATUS_DISABLE }}";
                $.post("{{ route('admin.applet.set') }}", {
                    id: id,
                    key: name,
                    value: val
                }, function (res) {
            ...
            table.on('toolbar({{ getDomIdKey('', 'table') }})', function (obj) {
                ...
                switch (obj.event) {
                    case 'custom_export':
                        var field = form.val('{{ getDomIdKey('', 'table_filter') }}');
                        location.href = '{{ route('admin.applet.list') }}?export=1&' + $.param(field);
                        break;
                }
            });
            ...
            table.on('tool({{ getDomIdKey('', 'table') }})', function (obj) { // 双击 toolDouble
                ...
                if (obj.event === 'edit') {
                    layer.open({
                        ...
                        type: 2,
                        area: ['80%', '80%'],
                        content: '{{ route('admin.applet.save') }}/' + data.id
                    });
