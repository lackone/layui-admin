        ...
        <div class="layui-card-body">
            <form class="layui-form layui-row layui-col-space16" lay-filter="{{ getDomIdKey('', 'table_filter') }}">
                <div class="layui-form-item">
                    @include('component.text', ['label' => '文章标题', 'name' => 'title', 'is_search' => 1])
        ...
        layui.use(['table', 'dropdown', 'laydate', 'form', 'util', 'jquery'], function () {
            ...
            table.render({
                ...
                cols: [[
                    ...
                    },
                    {
                        ...
                        title: '分类',
                        templet: function (d) {
                            let cate = JSON.parse('{!! json_encode(\App\Models\ArticleCategory::optionList('id', 'name')) !!}')
                            return cate[d.category_id] ? cate[d.category_id] : '';
                        }
                    },
                    {
                        field: 'banner',
                        ...
                        templet: function (d) {
                            if (d.banner) {
                                return '<img src="' + d.banner + '" style="width:36px;">';
                            } else {
                                return '';
                            }
                        }
                    ...
                    },
                    {
                        ...
                        title: '置顶',
                        templet: function (d) {
                            if (d.is_hot == '{{ \App\Models\Article::IS_HOT_ENABLE }}') {
                    ...
                    },
                    {
                        ...
                        title: '状态',
                        templet: function (d) {
                            if (d.status == '{{ \App\Models\Article::STATUS_ENABLE }}') {
                            ...
                            switch (obj.id) {
                                case '{{ getDomIdKey('', 'add') }}':
                                    layer.open({
                                        ...
                                        type: 2,
                                        area: ['80%', '80%'],
                                        content: '{{ route('admin.article.save') }}'
                                    });
                                    break;
                                case '{{ getDomIdKey('', 'update') }}':
                                    ...
                                    layer.open({
                                        ...
                                        type: 2,
                                        area: ['80%', '80%'],
                                        content: '{{ route('admin.article.save') }}/' + data[0].id
                                    });
                                    break;
                                case '{{ getDomIdKey('', 'delete') }}':
                                    ...
                                    layer.confirm('真的删除么?', function (index) {
                                        $.post("{{ route('admin.article.delete') }}", {
                                            id: ids
                                        }, function (res) {
                                            if (res.code == 200) {
            ...
            table.on('toolbar({{ getDomIdKey('', 'table') }})', function (obj) {
                ...
                switch (obj.event) {
                    case 'custom_export':
                        var field = form.val('{{ getDomIdKey('', 'table_filter') }}');
                        location.href = '{{ route('admin.article.list') }}?export=1&' + $.param(field);
                        break;
                }
            });
            ...
            table.on('tool({{ getDomIdKey('', 'table') }})', function (obj) { // 双击 toolDouble
                ...
                if (obj.event === 'edit') {
                    layer.open({
                        ...
                        type: 2,
                        area: ['80%', '80%'],
                        content: '{{ route('admin.article.save') }}/' + data.id
                    });
